...
**후속(2026-09-17, 단계 6 실측 중 발견) - 이 두 항목("재현 조건
미상"으로 남아있던 4번/⑤)에 유력한 설명이 새로 나왔다**: 브라우저
탭이 다른 곳으로 전환되면 Godot 웹 빌드의 메인 루프(`requestAnimationFrame`
기반)가 멈춰서 핑도 안 나가고, 15초 뒤 서버가 정상적으로 연결을 끊는다는
게 실측으로 확인됐다 - 게임이 길어질수록(정확히 "두 번째 게임 도중"이라는
기존 보고와 일치) 이 경로를 밟았을 개연성이 높다. **여전히 확정은
아니다**(다른 원인일 수도 있음). 자세한 조사/실측(2클라이언트 실제
재현 - 재접속은 이미 잘 되는 것까지 확인)은 `docs/multiplayer.md`
"§12. 웹 환경 고유 문제" 절 참고.
...
### 소스 공개 준비(README 작성 + 저장소 공개) - 작업 1: 안전 점검 결과
소스 저장소(`yacht-dice`)를 공개로 전환하기 전, 히스토리 전체를 대상으로
인증정보/개인정보/캐릭터 음성·이미지 파일을 점검했다.

- **인증정보(API 키/토큰/비밀번호/개인키)**: 94개 커밋 전체의 diff에서
  패턴 검색 - **0건.** 코드에 등장하는 "token"은 전부 재접속 토큰
  (`SessionStore`) 같은 변수명이었고 실제 비밀값은 없었다.
- **캐릭터 음성·이미지 파일**: 히스토리에 커밋된 미디어는
  `assets/sfx/*.wav`(효과음 4개, 아래 별도 처리 대상)와
  `assets/placeholder_portrait.png`(프로젝트 자체 기본 플레이스홀더
  이미지, 사용자가 올린 캐릭터 초상 아님 - 사용자 확인 받음)뿐이다 -
  **0건.**
- **개인정보 - 발견**: 파일 내용이 아니라 **git 커밋 메타데이터**에
  있었다. `git log --all --format='%an <%ae>'`로 확인한 결과 94개
  커밋 전부가 `KGB <Pandher0324@naver.com>`(로컬 `git config
  user.email`과 동일) - 파일 내용 grep으로는 안 걸리는 자리라 메타데이터까지
  따로 확인해야 했다.

**판단 - 과거 94개 커밋의 작성자 이메일은 그대로 둔다(히스토리 재작성
안 함).** 사용자가 확정한 근거 4가지:
1. 커밋에 작성자 이메일이 남는 것은 git의 기본 동작이고, 공개 저장소에서
   보편적인 상태다 - 실수로 유출된 비밀이 아니다.
2. 실질적 피해는 이메일 수집으로 인한 스팸 정도이고, 인증정보 유출과는
   성격이 다르다.
3. 고치는 비용은 구체적이다 - 작성자 이메일을 바꾸면(`git filter-repo`
   등) 94개 커밋 해시가 전부 바뀌는데, `docs/multiplayer.md`의 사고
   기록과 이 파일의 작업 이력 곳곳에 적어둔 해시 참조(`7da0e5c`,
   `dabe2ad` 등)가 전부 무효화된다 - 이 기록은 이 프로젝트에서 실제로
   반복해서 꺼내 쓴 자산이다.
4. 이메일 하나를 가리기 위해 그 기록의 추적성을 잃는 것은 교환이
   맞지 않는다.

**앞으로의 커밋만 영향받지 않게 조치**: 이 저장소 한정으로(`--global`
아님) `git config user.email`을 GitHub가 제공하는 no-reply 주소
(`설정 > Emails`의 "Keep my email addresses private"에서 확인)로
바꾼다 - 과거 커밋은 그대로 두되 앞으로 더 늘지 않게 하는 조치다.
"Block command line pushes that expose my email"도 함께 켜서, 나중에
다른 환경에서 `user.email`이 실수로 원래 값으로 되돌아간 채 커밋해도
GitHub가 푸시 자체를 거부하도록 이중 방어를 둔다. **이 항목이 나중에
"왜 안 고쳤지"로 오해되지 않도록 여기 이유를 남긴다** - 못 고친 게
아니라, 위 4가지 근거로 "고치는 비용 > 얻는 이득"이라고 판단해서 의도적으로
유지하기로 한 것이다.
