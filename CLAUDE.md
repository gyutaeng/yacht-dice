...
- **다음 계획**: 서버를 아직 상시 배포하지 않은 상태라, 지인 대상
  ...
  예정이다.
- **남은 것**: **2-7**(서버 상시 배포 - `docs/multiplayer.md`가 이미
  전제하고 있는 Render 무료 플랜으로, 클라이언트 쪽 재접속 백오프도
  이미 그 기준(유휴 시 최대 1분 걸려 깨어남)으로 맞춰져 있음 - 설계는
  끝났고 실제 배포만 남음), **Phase 3**(웹 퍼블리싱 마감 -
