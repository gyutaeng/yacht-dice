...
- **자체 구현으로는 계속 실패해서(3라운드) 검증된 애드온으로 교체했다.** `godot-file-access-web`(Scrawach, MIT License, 마지막 push 2025-12-30, 4.x 관련 이슈 보고 없음)의 핵심 스크립트를 `addons/FileAccessWeb/core/file_access_web.gd`에 수정 없이 그대로 벤더링(원본 확인: 콜백 객체를 멤버 변수에 저장 + `<input>`을 아예 DOM에 안 붙이고도 동작 — 우리가 겪은 GC 문제를 이미 우회한 코드였음). `scripts/io/file_picker_web.gd`는 이 애드온을 부르는 얇은 껍데기로 전면 교체 — `FilePicker` 공개 인터페이스(`create()`/`pick_files()`/`files_picked`/`pick_cancelled`/`debug_log`)와 `_finalize_pick()`의 확장자 재검사, 데스크톱 구현은 전부 그대로 유지.
  - **알려진 제약**: 원본 애드온은 파일 1개만 지원한다(`<input>`에 `multiple` 속성 없음, JS가 `files[0]`만 읽음) — 그래서 웹에서는 `pick_files(..., multiple=true)`를 호출해도 1개만 받고 경고 로그를 남긴다. 데스크톱은 영향 없음(FileDialog가 이미 다중 선택 지원).
  - 라이선스 표기: `addons/FileAccessWeb/LICENSE.txt`(MIT 전문 + 원본 링크). 배포 시 이 파일을 그대로 유지해서 같이 배포할 것.
  - 결과를 base64 문자열로 주는 원본 API(`loaded(file_name, file_type, base64_data)`)를 `Marshalls.base64_to_raw()`로 `PackedByteArray`로 바꿔 흡수했다. 6단계 화면 로그(`debug_log`)는 애드온 시그널(`load_started`/`progress`/`loaded`/`error`/`upload_cancelled`) 기준으로 다시 붙였다.
  - 브라우저 콘솔에 `print()`가 왜 안 보이는지 확인차 실제로 DevTools를 열어봤는데, `head_include`로 심어둔 순수 HTML `<script>`의 `console.log`는 보이는 반면(index.html:95 줄로 정확히 표시됨) Godot 엔진 자체의 시작 배너나 `JavaScriptBridge.eval()`로 찍는 로그는 전혀 안 보였다 — DevTools 콘솔의 컨텍스트 필터("top" 등)가 Godot의 실행 컨텍스트를 안 보여주고 있을 가능성이 있다. 원인을 완전히 못 밝혀서 사용자 안내대로 화면 로그(`_log_text`)를 계속 유일한 신뢰 채널로 쓴다.
  - 헤드리스 로드 + 전체 테스트(373개) 통과 확인, `F:/Godot/web_build/`로 재export 완료(빌드 시각은 실제 export 시점마다 달라짐 - 대화 로그 참고). **애드온 교체 후 실제 브라우저 클릭 테스트는 이 세션에서 자동화로 안정적으로 재현 못 해 사용자가 직접 확인하기로 함.**
