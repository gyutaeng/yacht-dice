...
**과거 사고 기록**: 저장소 루트에 캐릭터 아이콘 하나(`spr_betako_icon.png`,
595바이트, 2-4B 수동 검증 중 유입)가 실수로 커밋됐던 적이 있다 - 최신
커밋에서는 지웠지만(일반 삭제 커밋, force-push로 히스토리를 고치지
않았음 - 이 저장소가 곧 Render 배포 기준이 될 곳이라 히스토리를 건드는
위험이 595바이트 죽은 파일보다 훨씬 크다는 판단), **과거 커밋 히스토리에는
여전히 그 파일이 남아 있다.** 지금은 비공개라 문제 없지만, **이 저장소를
나중에 공개로 전환할 일이 생기면 이 기록부터 재검토할 것.** 재발 방지로
`.gitignore`에 저장소 루트 한정 이미지/오디오 확장자 규칙을 추가했다(아래
참고) - 하위 폴더(`assets/`, `characters/` 등)의 정상 에셋에는 영향 없음.
...
- **비공개 저장소(`gyutaeng/yacht-dice`)에 실수로 올라간 게 있는지 감사 -
  하나 발견해서 정리 완료**: `git ls-files`로 194개 트래킹 파일 전부를
  확인한 결과, 빌드 산출물(`.godot/`는 `.gitignore`로 제외, `web_build`/
  `web_dev`는 애초에 이 저장소 바깥 경로라 구조적으로 섞일 수 없음)/
  서버 로그(`F:\Godot\logs\`도 저장소 바깥)/Render·GitHub 인증정보
  (Dockerfile/.dockerignore/docs 전체 grep, `.claude/settings.json`
  확인 - 전부 없음)는 전부 깨끗했다. **딱 하나 예외**: 저장소 루트에
  `spr_betako_icon.png`(595바이트)가 커밋 b91ad89(2-4B, "온라인 캐릭터
  선택")부터 들어있었다 - 코드 어디서도 참조되지 않는 죽은 파일이고,
  이름과 크기로 볼 때 2-4B 수동 검증 중 캐릭터 아이콘 하나를 실수로
  프로젝트 루트에 두고 `git add .`류 포괄 추가에 쓸려 커밋된 것으로
  보인다. 사용자 판단으로 **일반 커밋으로만 삭제**했다(히스토리는
  그대로 - force-push로 히스토리를 고치는 위험이 595바이트 죽은
  파일보다 훨씬 크고, 이 저장소는 곧 Render 배포 기준이 될 곳이라는
  이유). 자세한 기록은 위 "저장소 2개" 절의 "과거 사고 기록" 참고.
  재발 방지로 `.gitignore`에 저장소 루트 한정 이미지/오디오 확장자
  규칙(`/*.png` 등, 하위 폴더는 영향 없음)을 추가하고, 기존 에셋
  (assets/, characters/ 등)이 여전히 정상 트래킹되는지 `git check-ignore`로
  확인했다. `git add .`/`git add -A` 금지 원칙도 CLAUDE.md 코드
  스타일에 명시했다(위 참고).
