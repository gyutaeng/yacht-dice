...
### 현재 전체 테스트 개수
610개 (`scripts/tests/test_runner.tscn`, 전부 통과).
...
- Phase 2: 온라인 멀티플레이 (2-1·2-3·2-4·2-4B·2-4C 완료 - 실제 주사위
  진행이 서버 권위로 동작하고, 내 캐릭터도 온라인에 연결됐고, GameEvents
  릴레이가 전수 조사를 거쳐 빠짐없이 나간다). 다음은 2-5(캐릭터 팩
  실시간 전송 - 지금은 남의 슬롯이 닉네임만 있어서 실루엣+무음) 또는
  2-6(연결 끊김 처리 - 재접속 토큰 실제 매칭, AFK 자동 진행, 턴 제한 시간).
