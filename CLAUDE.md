...
- 2-4B 실제 확인: 실제 보이스가 있는 캐릭터로 온라인 방을 만들어서 내
  턴에 "내 차례" 보이스가 실제로 들리는지, 초상화가 뜨는지 직접 확인.
- Phase 2: 온라인 멀티플레이 (2-1·2-3·2-4·2-4B 완료 - 실제 주사위 진행이
  서버 권위로 동작하고, 내 캐릭터도 온라인에 연결됨). 다음은 2-5(캐릭터
  팩 실시간 전송 - 지금은 남의 슬롯이 닉네임만 있어서 실루엣+무음)
  또는 2-6(연결 끊김 처리 - 재접속 토큰 실제 매칭, AFK 자동 진행, 턴
  제한 시간).
