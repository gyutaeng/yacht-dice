...
## 없다). 이미 방 안이므로 캐릭터 선택 직후 바로 전송한다. 해시가 지난
## 판과 같으면 전송 자체가 안 걸린다(2-5 캐시).
##
## **후보 A로 확정(1번 버그 검증 중 발견된 UX 충돌, 2026-09-17)** -
## 예전엔 여기서 `set_ready(true)`까지 자동으로 보냈는데, 상대가 이미
## 준비된 상태에서 이 화면에 들어오면 내가 실제로 새 캐릭터를 고르기도
## 전에(확정 버튼을 누르는 순간) 게임이 시작될 수 있었다 - 2-6B의
## "재대전 중에도 캐릭터를 바꿀 수 있다"는 결정과 충돌했다. 이제는
## 캐릭터 선택 확정 → 로비 화면으로 돌아옴 → 로비의 기존 [준비 완료]
## 버튼을 따로 누름, 세 단계로 나눠서 로비의 다른 모든 흐름(최초 입장
## 포함)과 완전히 같은 패턴으로 통일했다 - 조기 시작 여지 자체가
## 구조적으로 없어진다. 클릭이 한 번 늘어나는 게 유일한 대가.
