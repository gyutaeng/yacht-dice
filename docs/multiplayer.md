...
- 재접속에 성공하면 서버는 방 전원에게 `player_reconnected(player_index)`를
  보낸다.
- 재접속 유예 시간은 **60초**(`NetProtocol.RECONNECT_GRACE_MSEC`) - 이
...
**결정: 위 재접속 유예와 완전히 같은 원칙("연결이 끊기면 슬롯을 살려
재접속을 기다린다")을 그대로 적용한다** - `rematching` 전용의 별도
규칙을 새로 만들지 않는다. `RoomManager.remove_peer()`의 그레이스
판정에 `Room.State.REMATCHING`을 `TRANSFERRING`/`IN_GAME`과 나란히
추가하는 것으로 끝난다(`voluntary`가 아니면 슬롯을 비우지 않고
`GRACE_PERIOD`로 표시).

다만 "언제 포기하는지"는 `TRANSFERRING`/`IN_GAME`의 슬롯별 그레이스
타이머를 재사용하지 않고, **방 전체에 걸린 하나의 재대전 대기 시간**
(`NetProtocol.REMATCH_READY_TIMEOUT_MSEC`, 2분)으로 처리한다
(`Room.begin_rematch_wait()`/`is_rematch_wait_timed_out()`). **재접속
유예(`RECONNECT_GRACE_MSEC`)는 60초로 줄었지만 이 값은 그대로 2분이다**
(사용자 요청 범위 밖 - 재대전 대기는 게임이 이미 끝난 뒤라 게임 도중
끊김만큼 급하지 않다는 판단, 필요해지면 따로 조정). 이유:

- 재대전 대기 중에는 "연결은 멀쩡한데 `한 판 더`를 안 누른 사람"과
  "끊긴 채 안 돌아온 사람"을 굳이 구분할 이유가 없다 - 둘 다 결과는
  같다(다음 판을 시작 못 하게 막고 있다). 슬롯별 그레이스 타이머를 또
  만들면 이 두 경우를 각자 다른 타이머로 추적해야 해서 복잡도만 늘고
  실익이 없다.
- 대기 시간이 끝나면(`_service_rematch_rooms()`) 그때까지 준비 안 한
  슬롯을 전부(연결 여부 무관) 강제로 비운다(`RoomManager.force_vacate_slot()`) -
  `player_left(reason="timeout")`을 방송한 뒤 그 방은 다시 사람을
  기다리는 `rematching` 상태로 남는다(§3 - `rematching`도 신규 참가를
  받아준다). 완전히 나가는 것과 같은 취급이라 토큰도 같이 파기된다 -
  재접속 유예처럼 "이탈 후에도 토큰으로 돌아올 수 있다"는 여기선
  적용하지 않는다(대기 시간 자체가 이미 2분을 줬으므로 한 번 더 봐줄
  이유가 없다).
- 남은 시간은 1초 주기로 `player_timer(kind="rematch")`로 방송한다(§2.2) -
  ...
  카운트다운(2-5에서 겪은 그 문제)이 재발하지 않게 한다.
