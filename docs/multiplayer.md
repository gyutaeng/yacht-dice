...
## 12. 웹 환경 고유 문제 - 브라우저 탭 비가시화로 인한 연결 끊김 (조사만, 구현 안 함)

단계 6(연결 유지 시간 실측) 1차 시도에서 발견됨 - Render가 끊은 게
아니라 우리 서버가 핑 무응답(15초)으로 정상적으로 끊은 것이었다.
사용자가 방을 만든 뒤 Render 로그를 보려고 다른 탭으로 전환한 시점과
수신 중단 시점이 일치한다는 것을 근거로, "브라우저 탭이 안 보이면
Godot 웹 빌드의 메인 루프가 멈춘다"는 가설을 세우고 조사를 요청받았다.
공개 베타 전에 반드시 정리해야 할 항목으로 분류됨(트위터 공개 시
불특정 다수가 당연히 탭을 옮겨 다님).

### 1. Godot 웹 빌드는 탭이 안 보이면 실제로 멈추는가

**조사 방법의 한계부터 밝힌다**: 이 세션에는 실제 브라우저를 직접
조작할 도구(Chrome 자동화)가 연결돼 있지 않아서, "탭을 실제로 전환해
보고 눈으로 확인"하는 실측은 못 했다. 대신 **실제로 export한 웹 빌드의
런타임 코드(`index.js`)를 직접 열어 확인**하고, 거기서 확인된 사실에
**표준화되어 문서화된 브라우저 동작**(Page Visibility API)을 결합해서
결론을 냈다 - 추측이 아니라 코드 확인 + 스펙 확인의 조합이지만, 실제
브라우저 조작으로 눈으로 본 것은 아니라는 점을 구분해서 남긴다.

- `index.js`에서 Emscripten의 `MainLoop`가 `mode==1`(rAF 모드)로
  설정돼 있고 `MainLoop.scheduler`가 `requestAnimationFrame`을 직접
  호출하는 것을 확인했다 - Godot 웹 export가 기본적으로
  `requestAnimationFrame`으로 메인 루프(따라서 `_process()`/
  `_physics_process()` 전부)를 구동한다는 뜻이다.
- **`requestAnimationFrame` 콜백은 탭이 보이지 않을 때 모든 주요
  브라우저가 정지시키거나 심하게 억제한다** - 이건 Godot의 선택이
  아니라 Page Visibility API가 의도한 표준 브라우저 동작이다. `index.js`에
  `visibilitychange` 이벤트를 다루는 코드가 실제로 있긴 했지만
  `GodotWebXR`(VR/AR 세션) 전용이었다 - 우리 2D 게임과 무관, 일반
  세션의 메인 루프 스케줄링 방식을 바꾸지 않는다.
- `project.godot`/`export_presets.cfg`에 vsync/메인 루프 방식을 바꾸는
  설정이 전혀 없다(기본값 그대로) - Godot 프로젝트 설정 하나로 이
  동작을 끄는 방법은 없다(엔진/브라우저 구조 자체의 제약이라 프로젝트
  설정 레벨에서 우회할 수 있는 게 아님).
- **조건별 추정(Page Visibility 스펙 기준, 개별 실측은 못 함)**:
  - 같은 창 안에서 다른 탭으로 전환 → `document.hidden = true` →
    멈춤/억제.
  - 창을 최소화 → `document.hidden = true` → 멈춤/억제.
  - **다른 창에 가려지지만(z-order상 뒤) 그 창 자체는 최소화도
    안 됐고 탭 전환도 안 한 경우** → 표준 Page Visibility 구현은
    "가려짐"을 "숨김"으로 안 본다(탭 전환/최소화만 숨김으로 침) -
    즉 이 경우엔 rAF가 계속 돈다고 예상된다.
  - **별개 창 두 개를 겹치지 않게 나란히 띄운 경우**(사용자가 다음
    측정에서 쓰기로 한 방식) → 위와 같은 이유로 양쪽 다
    `document.hidden = false`로 유지될 것으로 예상된다 - 이번
    측정에서 유휴 스핀다운을 제대로 재기 위해 적절한 방법으로 보인다.

### 2. 핑 타임아웃 15초는 어디서 정했나

`scripts/net/protocol.gd`의 `PING_INTERVAL_MSEC=5000`/
`PING_TIMEOUT_MSEC=15000`(2-6에서 도입, 위 §6 "끊김 감지" 참고) -
근거는 **"네트워크가 응답 없이 사라지는 경우(와이파이 끊김 등)"**였다.
**탭이 백그라운드로 가서 메인 루프 자체가 멈추는 상황은 이 값을 정할
때 전혀 고려되지 않았다** - 순전히 "핑 간격의 3배 정도면 네트워크
지터에 대한 여유로 충분하다"는 판단이었고, 사람이 다른 탭/창을 얼마나
오래 보고 있을지는 애초에 이 상수의 설계 범위 밖이었다.

### 3. 대응 후보 비교

**(A) 핑 타임아웃을 늘린다**
- 얻는 것: 짧은 탭 전환(수 초~수십 초)을 견딤.
- 적절한 값이 없다는 게 근본 문제: 사람이 다른 탭/창을 얼마나 오래
  보는지는 예측 불가(알림 확인 몇 초일 수도, 딴짓 몇 분일 수도) -
  아무리 늘려도 그보다 긴 이탈에는 여전히 뚫린다. 문턱을 옮길 뿐
  근본 해결이 아니다.
- 잃는 것: 이 값은 **진짜 끊긴 상대**(와이파이 끊김, 브라우저 크래시,
  탭을 아예 닫음 등)를 감지하는 데도 똑같이 쓰인다 - 늘릴수록 진짜
  이탈 감지가 느려지고, 다른 플레이어들이 더 오래 기다리게 된다.
  재접속 유예(60초)와 합쳐지면 최악의 경우 대기 시간이 그만큼
  늘어난다(예: 핑 타임아웃을 60초로 늘리면 핑 타임아웃(60초)+
  그레이스(60초)=최악 120초 뒤에야 자동 처리).

**(B) 끊기는 것은 허용하고 복귀를 매끄럽게 한다 - 실측 결과: 이미
거의 완성돼 있다**

**실제로 검증함**(헤드리스 2클라이언트 시나리오, 오늘): 온라인 2인
게임을 실제로 시작한 뒤, 한쪽 `GameClient` 노드의 `_process()`를
17초간 강제로 멈춰서(탭이 백그라운드로 가 rAF가 멈추는 것과 정확히
같은 메커니즘 - 핑 응답도 끊김 감지도 전부 `_process()` 안에 있으므로
이 노드 하나를 멈추는 것으로 충실하게 재현된다) 서버의 핑 무응답
15초 처리를 그대로 유도했다. 결과:
- 서버가 정확히 15초 무응답 시점에 정상적으로 연결을 끊었다
  (`close_code=1000`, 정상 종료 프레임 - 강제 종료가 아니라 서버가
  의도적으로 닫은 것과 일치).
- **처리를 재개하자(탭이 다시 보이는 순간) `online_screen.gd`에 이미
  있던 "게임 화면에 한 번이라도 들어간 뒤의 `disconnected`는 로비로
  안 튕기고 조용히 재접속을 시도한다"(2-6) 로직이 그대로 발동해서
  자동으로 재접속에 성공했다**(`game_reconnected` 신호 수신).
- **재접속 후 실제로 게임을 계속할 수 있는지까지 확인** - 재접속
  직후 `request_roll()`을 보내 서버로부터 정상적인 `state_snapshot_received`
  응답을 받는 것까지 확인했다. 재접속이 "신호만 오고 실제로는 안
  이어지는" 겉치레가 아니라는 뜻이다.
- **결론: 새로 만들 필요가 없다 - 2-6에서 이미 만든 재접속 인프라가
  이 시나리오를 그대로 커버한다.**

**발견된 사소한 흠(기능 손상 없음, 정리할 가치는 있음)**: 처리 재개
직후 첫 프레임에서 엔진이
`ERROR: Condition "!peers_map.has(p_id)" is true. Returning:
Ref<WebSocketPeer>()`를 콘솔에 찍는다(`game_client.gd`의
`_flush_outgoing_queue()` → `_peer.get_peer(1)`, 이미 서버가 끊어서
엔진 내부 맵에서 사라진 peer ID를 조회해서 발생). **기능적으로는
무해하다** - 바로 다음 줄에서 정상적으로 연결 끊김이 감지되고
재접속도 그대로 성공했다. 원인은 `_flush_outgoing_queue()`가 큐가
비어있어도 매 프레임 무조건 `_peer.get_peer(1)`을 호출하기 때문 -
큐가 비어있으면 그 값을 애초에 안 쓰므로, 큐가 비었을 때는 호출
자체를 건너뛰면 이 노이즈를 없앨 수 있어 보인다. 다만 **실제 사용자의
브라우저 콘솔에도 이 빨간 ERROR가 그대로 찍힐 것**이므로("뭔가 잘못됐나?"
하는 불안을 줄 수 있음), 사소하지만 정리 후보로 남긴다.

**남는 체감 손실**: 재접속 중엔 화면에 배너가 뜨고(2-6에 이미 있음),
그 사이 다른 플레이어에게는 내 연결 상태가 "끊김"으로 표시된다 -
몇 초~수십 초 정도 상대가 잠깐 사라진 것처럼 보인다. 큰 손실은
아니지만 "아무 표시 없이 매끄럽게"는 아니다.

**(C) 탭 가시성 변화를 감지해서 사용자에게 알린다**
- 방향: JS `visibilitychange` 이벤트를 `JavaScriptBridge`로 GDScript에
  연결(§1에서 확인한 대로 `GodotWebXR`가 이미 비슷한 걸 하고 있어
  패턴 자체는 확립돼 있음) - 탭이 숨겨지는 순간 "다른 탭으로 가면
  게임이 멈추고 잠시 뒤 연결이 끊길 수 있습니다" 안내, 복귀 시
  "돌아왔습니다 - 재접속 중..." 같은 명확한 문구를 보여준다.
- 장점: 왜 끊겼는지 이해시켜서 당황("갑자기 왜 끊겼지?")을 줄인다 -
  근본 원인은 못 없애도 "고장"이 아니라 "이런 환경 제약"이라는 인식을
  심어준다.
- 단점: 그 자체로는 끊김을 막지 못한다 - (B)와 반드시 같이 가야
  의미가 있다.
- 참고: "숨겨지는 순간"과 "복귀하는 순간"은 감지 가능하다(그 프레임까지는
  아직 처리 중이거나, 복귀 직후 처리가 재개되며 큐잉된 JS 이벤트를
  받을 수 있음) - 다만 숨겨져 있는 "동안"에는 당연히 아무 GDScript
  코드도 못 돈다(그게 문제의 본질이므로).

**결정(사용자 확정, 2026-09-17) - 구현은 아직 안 함**:
- **(A) 핑 타임아웃 연장 - 채택 안 함.** 적절한 값이 없고, 진짜 이탈
  감지를 늦추는 대가만 치른다는 판단 그대로 확정.
- **(B) 끊김 허용 + 자동 재접속 - 기본 동작으로 확정, 추가 구현 없음.**
  오늘 실측(위)으로 이미 잘 동작함이 확인됐다 - 이대로 유지한다.
- **(C) 가시성 감지 + 사용자 안내 - 채택.** (B)만으로는 사용자가 무슨
  일이 있었는지 모른다 - 돌아왔을 때 "연결이 끊겼다가 복구되었습니다"
  같은 상태를 알 수 있어야 한다는 게 채택 이유. **구현 시점은
  `docs/deployment_checklist.md`의 배포 순서 절 참고(보류 항목으로
  분류, 공개 베타 필수 항목은 아님).**

### 4. 턴 제한 시간(60초)과의 상호작용

핑 타임아웃(15초) < 턴 제한(60초)이므로, **자기 턴 도중 탭이 숨겨지면
서버가 먼저(15초 시점) "연결 끊김"으로 감지**하고, 이어서 재접속
유예(IN_GAME 60초)가 시작된다. 이 유예와 턴 제한(턴이 시작된 시각
기준으로 독립적으로 흐름)은 2-6에서 이미 "겹쳐도 정확히 한 번만
`auto_confirm_least_damaging()`이 불린다"고 검증된 OR 조건으로
묶여 있다(§6 "턴 제한 시간" 항목, 경계값 테스트까지 있음) - 이 부분은
새로 검증할 필요가 없다.

- **유예(60초) 안에 탭이 다시 보이면**: 위 (B) 실측대로 자동
  재접속되고, 그 시점까지 아직 자기 턴이 살아있다면(턴 시작 후 60초를
  안 넘겼다면) 이어서 진행 가능.
- **유예를 넘기면**: 이미 자동으로 확정 처리된 뒤라 복귀해도 그 턴은
  되돌릴 수 없다 - "게임이 막히지 않는다"는 기존 설계 원칙 그대로
  적용된 것뿐이라 이상 동작이 아니다.

### 5. 기존 미해결 베타 버그와의 연결 (확정 아님, 유력한 설명)

`CLAUDE.md`에 "④'(정정) - 두 번째 게임 도중 서버 접속이 끊김"(재현
조건 미상으로 남아있던 항목)과 "⑤ - 게임 종료 후 새로고침하면 방으로
복귀 못 함" 둘 다, 이번에 확인된 메커니즘(탭을 다른 곳으로 돌리면
15초 뒤 핑 무응답으로 서버가 끊음)이 유력한 설명이 될 수 있다 - 게임이
길어질수록(특히 "두 번째" 게임 도중이라는 보고와 일치 - 첫 판보다
시간이 더 지난 시점이라 중간에 딴 곳을 볼 가능성이 커짐) 이 경로를
밟았을 개연성이 높다. **확정은 아니다**(다른 원인일 수도 있음) - 하지만
"재현 조건 불명"으로 오래 남아있던 두 항목에 대해 지금까지 나온 것
중 가장 그럴듯한 단일 설명이라 기록해둔다. 다음에 실제로 재현되면
클라이언트 콘솔에 `[전송] [연결끊김] close_code=... 마지막 수신 후
15.0초` 근처 값이 찍히는지가 이 가설을 확인하는 단서가 될 것이다.
