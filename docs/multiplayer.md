...
**아직 못 찾은 것**: 애초에 왜 중간 청크 하나가 상대에게 안 갔는지(A가
안 보냈는지, 서버가 릴레이했는데 B가 못 받았는지)의 근본 원인은 이번에
안 밝혔다 - 재현할 때는 일부러 안 보낸 것이라 "안 보낸 경우 시스템이
버티는지"만 확인했다. 실제 원인 규명은 다음에 로그를 더 모아서 진행한다.
다만 원인이 무엇이든 이제는 그 결과(어느 클라이언트가 영원히 못 끝남)로
게임 진행 자체가 막히지는 않는다.
...
### 8.5-6 결측의 진짜 원인 조사 — 웹 빌드 받는 쪽 버퍼(진행 중)

§8.5-5의 재전송으로 임시 방편은 마련했지만, 원래 원인은 여전히 몰랐다.
사용자가 실제 웹 빌드(브라우저 2개)로 재현해서 가져온 로그를 분석한 결과:

- 결측이 무작위가 아니라 규칙적이었다(42개 중 16개, 대부분 홀수 인덱스).
- 방향이 한쪽뿐이었다 - 서버가 받는 클라이언트→서버 청크는 42/42 전부
  도착했는데, 클라이언트가 받는 서버→클라이언트 청크만 절반 가까이
  사라졌다.

**중요한 자각**: 이 세션의 모든 실제 소켓 검증(§8.5-1~8.5-5 전부 포함)은
서버·클라이언트 둘 다 네이티브 프로세스로 붙여서 했다. 그런데 서버는
`docs/multiplayer.md` §0에 이미 못박힌 대로 항상 네이티브 headless
...
**유력 가설(사용자 제시, 숫자가 맞아떨어짐)**: 청크 32KB를 Base64로
감싸면 약 43.8KB인데, `WebSocketPeer`의 받는 쪽 버퍼(`inbound_buffer_size`)
기본값이 65,535바이트라 한 번에 1.46개밖에 못 담는다 - 두 개가 연달아
도착하면 두 번째가 밀려날 수 있다. 예상 손실률(약 32%)이 실제 로그
(38%)와 비슷하고, "하나 걸러 하나씩"이라는 규칙적인 무늬도 무작위 유실이
아니라 버퍼가 찰 때마다 밀려나는 것이라면 설명된다.

**검증 계획(3단계, 아직 결과 안 나옴)**:
1. `NetProtocol.CLIENT_INBOUND_BUFFER_BYTES`(1MB)를 `GameClient.connect_to_server()`
   에서 `_peer.set_inbound_buffer_size()`로 설정 - `create_client()` **전에**
   설정해야 반영됨(네이티브에서 직접 확인). 설정 직후와 연결 완료 직후
   두 시점 모두 `get_inbound_buffer_size()`로 되읽어 로그를 남긴다 - 웹
   export에서 이 설정 자체가 무시될 가능성까지 확인하기 위함(네이티브
   되읽기는 정상 확인됨, 웹에서의 실제 반영 여부는 미확인).
2. 그래도 손실이 남으면 `NetProtocol.CHUNK_PAYLOAD_BYTES`(현재 32768)를
   8192로 낮춘 빌드로 재시도 - 서버는 이 상수를 전혀 참조하지 않고
   클라이언트가 보낸 `sequence`/`total_chunks`를 그대로 릴레이하므로,
   이 상수 하나만 바꾸면 서버 쪽 변경 없이 그대로 적용된다(확인함).
3. 둘 다 효과 없으면 원인이 이 설정들로 못 건드리는 더 아래 계층(브라우저
   WebSocket 구현 자체)에 있다는 뜻이므로 그때 다시 판단한다.

**게임 메시지 크기도 같이 확인**: 4인 게임에서 전 칸 확정 상태의
`state_snapshot`을 실측한 결과 693바이트로, 버퍼 상한과는 비교가 안 될
만큼 작다 - 이 버그가 실제 게임 진행 메시지(주사위/점수)에 당장 영향을
줄 정도는 아니라는 뜻이지만, 재전송 메커니즘이 없는 건 여전히 잠재
위험이다(사용자 지적, §7 참고).

이 버퍼 크기 조정은 **원인 규명을 위한 진단 빌드**다 - 결과가 나오기
전까지는 청크 크기(1900개로 늘어나는 문제 등)를 미리 고치지 않는다.
