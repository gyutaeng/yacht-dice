...
**조합(방향 제시만, 결정 안 함)**: (B)는 오늘 실측으로 이미 사실상
완성돼 있음이 확인됐다 - 추가로 의미 있는 건 (C)의 가벼운 안내(사용자가
당황하지 않게 하는 것)와, (B)에서 발견된 사소한 엔진 ERROR 로그 정리
정도다. (A)는 근본 해결이 아니고 진짜 이탈 감지를 느리게 만드는
트레이드오프가 있어 우선순위가 낮다고 판단된다. 서로 배타적이지 않으므로
조합 자체는 자유롭게 가능 - **결정과 구현은 이후로 미룬다.**
