...
- **설계 확정(2-6) - 확정 이탈 이후에도 재접속은 계속 허용한다.** 이
  문서를 처음 쓸 때는 명시하지 않았던 부분이다. 유예 2분이 끝나면 그
...
- **명시적으로 나가는 것(`leave()`)은 게임 도중이라도 유예를 안 거친다.**
  ...
  자리에서 슬롯을 완전히 비우고 토큰도 같이 파기한다(2분을 기다렸다가
  확정 이탈로 넘어가는 절차 자체를 생략). 스스로 나가겠다고 한 사람을
  2분 더 기다려줄 이유가 없기 때문이다.
...
(`NetProtocol.REMATCH_READY_TIMEOUT_MSEC`, 2분 - 재접속 유예와 같은
값)으로 처리한다(`Room.begin_rematch_wait()`/`is_rematch_wait_timed_out()`).
이유:
...
- 대기 시간이 끝나면(`_service_rematch_rooms()`) 그때까지 준비 안 한
  ...
  적용하지 않는다(대기 시간 자체가 이미 재접속 유예와 같은 2분을
  줬으므로 한 번 더 봐줄 이유가 없다).
